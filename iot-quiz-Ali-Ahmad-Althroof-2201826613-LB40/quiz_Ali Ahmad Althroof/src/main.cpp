...
void taskSendWarning(void *pvParameters) {
  while (1) {
    ...
    if (lux > 400) {
      client.publish("ESP_ALI/doorstate", "Door open");
    } else if (lux <= 400) {
      client.publish("ESP_ALI/doorstate", "Door closed");
    }
...
void loop() {
  ...
  if (millis() - lastTempPublish > 5000) {
    ...
    client.publish("ESP_ALI/temperature", tempStr);
  ...
  if (millis() - lastHumidityPublish > 6000) {
    ...
    client.publish("ESP_ALI/humidity", humidityStr);
  ...
  if (millis() - lastLightPublish > 4000) {
    ...
    client.publish("ESP_ALI/light", lightStr);
